const gameApp = {
    ...
    switchView(viewName) {
        ...
        // Determine which sidebar button should be active for hub views
        let activeSidebarBtnName = viewName;
        if (['brand', 'cinema', 'fashion', 'subsidiary'].includes(viewName)) activeSidebarBtnName = 'jobs';
        if (['spa', 'pr'].includes(viewName)) activeSidebarBtnName = 'condo';
        if (['sponsor'].includes(viewName)) activeSidebarBtnName = 'achievements';

        const selectedBtn = document.getElementById(`nav-btn-${activeSidebarBtnName}`);
        const selectedView = document.getElementById(`${viewName}-view`);
        
        if(selectedBtn) {
            selectedBtn.classList.add('active');
            const titleEl = document.getElementById('current-view-title');
            if(titleEl) titleEl.innerText = selectedBtn.querySelector('.nav-text').innerText;
        }
        ...
        if(viewName === 'cinema') this.renderCinema();
        if(viewName === 'fashion') this.renderFashion();
        if(viewName === 'subsidiary') this.renderSubsidiary();
        if(viewName === 'pr') this.renderPR();
        if(viewName === 'sponsor') this.renderSponsors();
        if(viewName === 'techtree') this.renderTechTree();
    },
    ...
    triggerUndergroundJob(jobType) {
        ...
        this.showDialog({
            ...
            type: "confirm",
            onConfirm: async () => {
                this.startTask('ai_dark_job', `Đang thực hiện giao dịch ngầm (${jobName})...`);
                const prompt = `Cô nàng người mẫu "${idol.name}" (Fame: ${idol.stats.fame}, Tha hóa: ${corr}, Stress: ${stress}) vừa bị ép tham gia công việc ngầm: "${jobName}".
Yêu cầu:
Viết 1 đoạn văn ngắn (3-4 câu) bằng tiếng Việt mô tả chân thực, u tối về buổi làm việc này trong góc khuất ngành giải trí. Tả tâm lý cô ấy, ánh đèn, những ánh mắt nguy hiểm hoặc bối cảnh xung quanh. Liệu cô ấy chịu đựng hay có sự cố nhỏ?
Dựa vào đó, đưa ra hệ số thưởng tiền "rewardMod" (0.8 đến 1.5) và hệ số rủi ro scandal "scandalMod" (0.5 đến 2.0).
Trả về CHỈ một JSON hợp lệ: {"story": "...", "rewardMod": 1.1, "scandalMod": 1.2}`;

                let resultJson = { story: "Một công việc trong buổi tối đen tối, mọi thứ diễn ra theo kế hoạch.", rewardMod: 1.0, scandalMod: 1.0 };
                try {
                    const resultText = await apiManager.generateText(prompt);
                    const cleanedStr = resultText.replace(/```json/g, '').replace(/```/g, '').trim();
                    const parsed = JSON.parse(cleanedStr);
                    if (parsed.story && parsed.rewardMod) {
                        resultJson = parsed;
                    }
                } catch (e) {
                    console.warn("AI Dark Job Gen failed", e);
                }
                this.endTask('ai_dark_job');

                const finalReward = Math.floor(moneyReward * (resultJson.rewardMod || 1));
                const finalScandalCost = Math.floor(scandalCost * (resultJson.scandalMod || 1));

                gameManager.updateMoney(finalReward);
                ...
                idol.scandalRisk = (idol.scandalRisk || 0) + finalScandalCost;
                if (idol.scandalRisk >= 100) {
                ...
                if (idol.corruption >= 100 && (!idol.concept || !idol.concept.includes("Dark Muse"))) {
                    ...
                    darkMuseStr = `<br><span style="color:var(--error); font-weight:bold;">⚠️ ${idol.name} CẢNH BÁO ĐÃ BIẾN THÀNH DARK MUSE!</span>`;
                }
                ...
                this.showDialog({
                    title: `Kết Quả: ${jobName}`,
                    message: `
                        <div style="font-size: 14px; color: #f87171; margin-bottom: 15px; border-left: 3px solid #dc2626; padding-left: 10px; background: rgba(0,0,0,0.5); padding: 10px;">
                            ${resultJson.story}
                        </div>
                        <strong>Thù lao nhận được:</strong> <span style="color:var(--success);">+${finalReward} 💰</span> <em>(Hệ số x${resultJson.rewardMod || 1})</em><br>
                        <strong>Rủi ro Scandal tăng:</strong> <span style="color:var(--error);">+${finalScandalCost}%</span> <em>(Hệ số x${resultJson.scandalMod || 1})</em><br>
                        <strong>Độ tha hóa:</strong> <span style="color:var(--error);">+${corrGain} | Căng thẳng: +${stressCost}</span>
                        ${darkMuseStr}
                    `,
                    type: "info"
                });
    ...
    async generateRivalNews() {
        ...
        try {
            let jsonString = await apiManager.generateText(prompt);
...
Output ONLY the dialogue text directly, no quotes around it, no explanations.`;
        ...
        this.injectKeys();
        try {
            let result = await apiManager.generateText(promptText);
  ...
  "reportMessage": "Trời ơi làm cái này cực quá sếp ạ, nhưng tiền nhiều thật!"
    ...
    _checkLevelUp(idol) {
        ...
        if (idol.exp >= 100) {
            ...
            this.triggerLevelUpEffect(idol.name, idol.level);
            if (typeof gameManager !== 'undefined') gameManager.checkAchievement('LEVEL_UP', idol);
        }
        return levelUpStr;
    },

    triggerLevelUpEffect(idolName, newLevel) {
        ...
        glow.style.background = 'radial-gradient(circle at center, rgba(250,204,21,0.4) 0%, transparent 70%)';
        glow.style.animation = 'glowPulse 1.5s ease-out forwards';
        
        const text = document.createElement('div');
        text.innerHTML = `⭐ ${idolName} LEVEL UP! (${newLevel}) ⭐`;
        text.style.position = 'absolute';
        text.style.top = '50%';
        text.style.left = '50%';
        text.style.transform = 'translate(-50%, -50%)';
        text.style.color = 'var(--gold, #facc15)';
        text.style.fontSize = 'min(40px, 8vw)';
        text.style.fontFamily = "'JetBrains Mono', monospace";
        text.style.whiteSpace = "nowrap";
        text.style.fontWeight = '900';
        text.style.textShadow = '0 0 20px rgba(250,204,21,1), 0 0 40px rgba(250,204,21,0.5), 0 0 60px rgba(250,204,21,0.3)';
        text.style.animation = 'textPopUp 2.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards';
        
        glow.appendChild(text);
        ...
        if (!document.getElementById('level-up-style')) {
            ...
            style.innerHTML = `
                @keyframes glowPulse {
                    0% { opacity: 0; background: radial-gradient(circle at center, rgba(250,204,21,0.8) 0%, transparent 50%); }
                    30% { opacity: 1; background: radial-gradient(circle at center, rgba(250,204,21,0.4) 0%, transparent 80%); }
                    100% { opacity: 0; background: radial-gradient(circle at center, rgba(250,204,21,0) 0%, transparent 100%); }
                }
                @keyframes textPopUp {
    ...
    openUseItem(itemId) {
        // Đóng ngay hộp thoại hiện tại để hộp thoại chọn Model có thể hiển thị
        const dialogModal = document.getElementById('dialog-modal');
        if (dialogModal.style.display === 'flex') {
    ...
    async executeUseItem(itemId, idolId) {
        ...
        if (item.type === 'stress') {
            ...
            idol.stats.fame = (idol.stats.fame || 0) + item.effect;
    ...
    async executeJob(jobId, idolId) {
        ...
        this.startTask('ai_job_exec', `Đang theo chân ${idol.name} đi làm (${job.name})...`);
        const prompt = `Cô nàng người mẫu "${idol.name}" (Fame: ${idol.stats.fame}, Stress: ${idol.stress}) vừa tham gia công việc: "${job.name}".
Yêu cầu:
Viết 1 kịch bản ngắn (3-4 câu) bằng tiếng Việt mô tả chân thực buổi làm việc này trong ngành giải trí. Tả phong thái, đạo cụ hoặc trang phục cô ấy mặc, và phản ứng của đạo diễn/khách hàng. Liệu có xảy ra sự cố hay thành công rực rỡ?
Dựa vào câu chuyện, đưa ra hệ số thưởng tiền (rewardMod) từ 0.5 (tệ) đến 2.0 (viral), hệ số mệt mỏi (stressMod) từ 0.5 (nhàn) đến 1.5 (vắt kiệt sức).
Trả về CHỈ một JSON hợp lệ với cấu trúc sau, tuyệt đối không có markdown block hay text dư thừa ngoài JSON:
{
  "story": "...",
  "rewardMod": 1.2,
  "stressMod": 1.0,
  "status": "success"
        ...
        let resultJson = { story: "Công việc diễn ra suôn sẻ và bình thường.", rewardMod: 1.0, stressMod: 1.0, status: "success" };
        try {
            const resultText = await apiManager.generateText(prompt);
            const cleanedStr = resultText.replace(/```json/g, '').replace(/```/g, '').trim();
            const parsed = JSON.parse(cleanedStr);
            ...
            console.warn("AI Job Gen failed, using defaults", e);
        }
        this.endTask('ai_job_exec');

        const finalReward = Math.floor(job.reward * (resultJson.rewardMod || 1));
        const finalStress = Math.floor(job.stress * (resultJson.stressMod || 1));
        const finalExp = job.exp;

        // Update stats
        idol.stress = (idol.stress || 0) + finalStress;
        idol.exp = (idol.exp || 0) + finalExp;
        ...
        if (idol.measurements && idol.measurements.weight) {
            ...
            if (match && Math.random() < 0.6) { // 60% chance to lose 1 kg
                ...
                newVal = Math.round(newVal * 10) / 10;
                idol.measurements.weight = currentStr.replace(match[0], newVal);
                weightDropStr = "<br><span style='color:var(--gold);'>* Cân nặng giảm do làm việc tần suất cao!</span>";
            }
        }
        ...
        let statusIcon = "✅";
        if (resultJson.status === "issue" || resultJson.rewardMod < 1) statusIcon = "⚠️";
        if (resultJson.status === "viral" || resultJson.rewardMod >= 1.5) statusIcon = "🔥";

        this.showDialog({
            title: `Kết Quả Hợp Đồng: ${job.name}`,
            message: `
                <div style="font-size: 14px; color: var(--text-main); margin-bottom: 15px; border-left: 3px solid var(--primary); padding-left: 10px; background: rgba(0,0,0,0.2); padding: 10px;">
                    ${resultJson.story}
                </div>
                <strong>Đánh giá:</strong> ${statusIcon} ${resultJson.status.toUpperCase()}<br>
                <strong>Thù lao nhận được:</strong> <span style="color:var(--success);">+${finalReward} 💰</span> <em>(Hệ số x${resultJson.rewardMod || 1})</em><br>
                <strong>Mức độ stress:</strong> <span style="color:var(--error);">+${finalStress}%</span><br>
                <strong>Kinh nghiệm:</strong> <span style="color:var(--primary);">+${finalExp} EXP</span><br>
                ${levelUpStr ? `<div style="color:var(--gold); margin-top:5px; font-weight:bold;">${levelUpStr}</div>` : ''}
                ${weightDropStr}
            `,
            type: "info"
        });
    ...
    openInventory(filterCat = null) {
        ...
        const cats = [
            ...
            { id: 'premium', name: 'Đặc Biệt' }
        ];

        let tabsHtml = '';
        cats.forEach(c => {
            const activeStyle = c.id === activeCat 
                ? 'background: var(--gold); color: black;' 
                : 'background: rgba(255,255,255,0.05); color: var(--text-muted); border: 1px solid rgba(255,255,255,0.1);';
            tabsHtml += `<button class="btn-action" style="${activeStyle} padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: bold; white-space: nowrap; margin: 0; transition: all 0.2s;" onclick="gameApp.openInventory('${c.id}')">${c.name}</button>`;
        });
        document.getElementById('inventory-tabs').innerHTML = tabsHtml;

        let itemsHtml = `<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 15px;">`;
        let hasItems = false;
        ...
        Object.keys(inventory).forEach(itemId => {
            ...
            if (qty > 0) {
                ...
                if (itemDef && (activeCat === 'all' || itemDef.category === activeCat)) {
                    hasItems = true;
                    // Split emoji and name if possible
                    let emoji = '📦';
                    let title = itemDef.name;
                    const nameParts = itemDef.name.match(/^([\p{Emoji}]+)\s*(.*)/u);
                    if (nameParts) {
                        emoji = nameParts[1];
                        title = nameParts[2];
                    } else {
                        // Fallback emoji by category
                        if(itemDef.category === 'health') emoji = '🥤';
                        if(itemDef.category === 'beauty') emoji = '✨';
                        if(itemDef.category === 'gift') emoji = '🎁';
                        if(itemDef.category === 'training') emoji = '🎓';
                        if(itemDef.category === 'premium') emoji = '💎';
                    }

                    itemsHtml += `
                    <div style="background: linear-gradient(145deg, var(--bg-surface), #1a1a1a); padding: 15px; border-radius: 12px; border: 1px solid var(--border-color); display: flex; flex-direction: column; position: relative; box-shadow: 0 4px 10px rgba(0,0,0,0.3);">
                        <div style="position: absolute; top: -10px; right: -5px; background: var(--primary); color: white; padding: 2px 10px; border-radius: 12px; font-size: 13px; font-weight: bold; border: 2px solid var(--bg-elevated); box-shadow: 0 2px 5px rgba(0,0,0,0.5);">x${qty}</div>
                        <div style="font-size: 40px; text-align: center; margin-bottom: 5px; filter: drop-shadow(0 4px 6px rgba(0,0,0,0.5));">${emoji}</div>
                        <strong style="color: var(--gold); text-align: center; margin-bottom: 5px; font-size: 14px; min-height: 40px; display: flex; align-items: center; justify-content: center;">${title}</strong>
                        <div style="font-size: 11px; color: var(--text-muted); text-align: center; margin-bottom: 15px; flex-grow: 1; border-top: 1px dashed rgba(255,255,255,0.1); padding-top: 8px;">${itemDef.desc}</div>
                        <button class="btn-action w-full" style="padding: 8px; background: rgba(52,211,153,0.1); border: 1px solid var(--success); color: var(--success); margin: 0; font-weight: bold; border-radius: 6px; transition: all 0.2s;" onmouseover="this.style.background='var(--success)'; this.style.color='black';" onmouseout="this.style.background='rgba(52,211,153,0.1)'; this.style.color='var(--success)';" onclick="document.getElementById('inventory-modal').style.display='none'; gameApp.openUseItem('${itemId}')">SỬ DỤNG</button>
                    </div>`;
                }
            }
        });
        ...
        if (!hasItems) itemsHtml = `
            <div style="text-align:center; padding: 60px 20px; color: var(--text-muted); background: rgba(255,255,255,0.02); border-radius: 12px; border: 1px dashed rgba(255,255,255,0.1);">
                <div style="font-size: 40px; margin-bottom: 15px; opacity: 0.5;">🛒</div>
                Khu vực phân loại này đang trống rỗng.<br>Hãy mua sắm để nạp thêm vật tư!
                <br><br>
                <button class="btn-action mt-10" onclick="document.getElementById('inventory-modal').style.display='none'; gameApp.switchView('shop');">ĐI LƯỢN MUA SẮM</button>
            </div>
        `;

        document.getElementById('inventory-content').innerHTML = itemsHtml;
    ...
    setApiState(text, stateClass = '') {
        const widget = document.getElementById('api-minimal-status');
        const stateEl = document.getElementById('api-state-text');
        if (!widget || !stateEl) return;
        
        stateEl.textContent = text;
        widget.className = 'api-widget-minimal';
        if (stateClass) {
            widget.classList.add(stateClass);
        }
    },

    updateApiStatusWidget() {
        // Obsolete, left empty to avoid breaking syncUI
    },
    ...
    async init() {
        // Adaptive Performance/Feature Gating
        const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
        const isLowEnd = typeof navigator.deviceMemory !== 'undefined' ? navigator.deviceMemory <= 4 : (navigator.hardwareConcurrency <= 4);
        ...
        if (isLowEnd) {
            ...
            this.showToast("Đã bật chế độ Tối ưu hiệu năng cho thiết bị hiện tại.", "info");
        } else {
            document.body.classList.add('perf-mode-high');
        }
    ...
    async triggerAIScout(tier = 'street') {
        ...
        let candidate = null;
        try {
            candidate = await cardEngine.generateAIModelProfile(query, tier);
        } catch (error) {
	...
	renderCards() {
        ...
        let baseArray = Array.from(cardEngine.roster.values());
        let totalFans = 0;
        let avgFame = 0;
        let avgStress = 0;
        if(baseArray.length > 0) {
            totalFans = baseArray.reduce((acc, idol) => acc + (idol.fans || 0), 0);
            avgFame = Math.round(baseArray.reduce((acc, idol) => acc + (idol.stats?.fame || 0), 0) / baseArray.length);
            avgStress = Math.round(baseArray.reduce((acc, idol) => acc + (idol.stress || 0), 0) / baseArray.length);
        }
            ...
            if (baseArray.length > 0) {
                countBadge.innerHTML = `
                    <div style="display:flex; gap:15px; text-align:center;">
                        <div>
                            <div style="font-size:10px; color:rgba(0,0,0,0.6); text-transform:uppercase; font-weight:800;">TỔNG FANS</div>
                            <div style="font-size:16px; font-weight:900;">${totalFans >= 1000000 ? (totalFans/1000000).toFixed(1)+'M' : (totalFans >= 1000 ? (totalFans/1000).toFixed(1)+'K' : totalFans)}</div>
                        </div>
                        <div style="width:1px; background:rgba(0,0,0,0.2);"></div>
                        <div>
                            <div style="font-size:10px; color:rgba(0,0,0,0.6); text-transform:uppercase; font-weight:800;">AVG STRESS</div>
                            <div style="font-size:16px; font-weight:900; color:${avgStress > 50 ? '#b91c1c' : '#000'}">${avgStress}%</div>
                        </div>
                        <div style="width:1px; background:rgba(0,0,0,0.2);"></div>
                        <div>
                            <div style="font-size:10px; color:rgba(0,0,0,0.6); text-transform:uppercase; font-weight:800;">QUY MÔ</div>
                            <div style="font-size:16px; font-weight:900;">${baseArray.length} MODELS</div>
                        </div>
                    </div>
                `;
            } else {
        ...
        let rosterArray = Array.from(cardEngine.roster.values());
        
        // Filter logic
        const searchInput = document.getElementById('roster-search');
        if (searchInput && searchInput.value) {
            const query = searchInput.value.toLowerCase().trim();
            rosterArray = rosterArray.filter(idol => {
                if (!idol) return false;
                const nameMatch = (idol.name || '').toLowerCase().includes(query);
                const conceptMatch = (idol.concept || '').toLowerCase().includes(query);
                const natMatch = (idol.nationality || '').toLowerCase().includes(query);
                return nameMatch || conceptMatch || natMatch;
            });
        }

        const tierFilter = document.getElementById('roster-tier-filter');
        if (tierFilter && tierFilter.value !== 'ALL') {
            const selectedTier = tierFilter.value;
            rosterArray = rosterArray.filter(idol => {
                const tierObj = this.getModelTier(idol);
                return tierObj.id.includes(selectedTier); // Includes handles things like 'S', 'S+', etc if they exist
            });
        }
        
        let htmlBuffer = '';

        // Sorting logic based on select value
        const sortSelect = document.getElementById('roster-sort');
        if (sortSelect) {
            const sortMode = sortSelect.value;
            rosterArray.sort((a, b) => {
                if (sortMode === 'fame_desc') {
                    return (b.stats?.fame || 0) - (a.stats?.fame || 0);
                } else if (sortMode === 'fame_asc') {
                    return (a.stats?.fame || 0) - (b.stats?.fame || 0);
                } else if (sortMode === 'stress_desc') {
                    return (a.stress || 0) - (b.stress || 0); // Ít stress (nghỉ ngơi nhiều) lên đầu
                } else if (sortMode === 'stress_asc') {
                    return (b.stress || 0) - (a.stress || 0); // Áp lực cao lên đầu
        ...
        if (rosterArray.length === 0 && baseArray.length > 0) {
            htmlBuffer = `<div style="grid-column: 1/-1; text-align: center; padding: 60px; color: var(--text-muted); background: rgba(0,0,0,0.2); border-radius: 12px; border: 1px dashed rgba(255,255,255,0.1);">
                <div style="font-size: 30px; margin-bottom: 10px; opacity: 0.5;">🔎</div>
                Không tìm thấy Model nào khớp với điều kiện lọc & tìm kiếm.
            </div>`;
        } else {
            rosterArray.forEach(idol => {
                if (!idol) return; 
            ...
            htmlBuffer += `
                <div class="idol-card" id="card-${idol.id}" style="padding:0; overflow:hidden; border: 1px solid transparent; border-radius: var(--radius-md); box-shadow: 0 4px 20px rgba(0,0,0,0.4); display: flex; flex-direction: column; cursor: pointer; position: relative; transition: all 0.4s cubic-bezier(0.16, 1, 0.3, 1);" onclick="gameApp.openIdolProfile('${idol.id}')" onmouseover="if(!document.body.classList.contains('perf-mode-low')) { this.style.transform='translateY(-8px)'; this.style.boxShadow='0 15px 35px rgba(212,175,55,0.2)'; this.style.borderColor='var(--gold)'; }" onmouseout="if(!document.body.classList.contains('perf-mode-low')) { this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 20px rgba(0,0,0,0.4)'; this.style.borderColor='transparent'; }">
                    ...
                    <div style="width: 100%; height: 220px; position:relative;">
                         ...
                         <div style="position:absolute; bottom:0; padding:60px 15px 15px; width:100%; background: linear-gradient(to top, rgba(0,0,0,1) 0%, rgba(0,0,0,0.8) 40%, transparent 100%);">
                             ...
                             <div style="font-size:12px; color:var(--primary); font-weight:800; text-transform: uppercase;">[ ${idol.nationality || 'Unknown'} ]</div>
                             <div style="font-size:11px; color:#ddd; font-weight:700; margin-top:2px; line-height: 1.4; text-transform: uppercase;">${idol.concept || 'Trainee'}</div>
                         </div>
                    </div>
                    
                    <div style="padding: 16px; background: rgba(20,20,25,0.95); flex: 1; display:flex; flex-direction:column; gap:12px; border-top: 2px solid ${isDarkMuse ? '#ff4d4d' : 'var(--gold)'}; backdrop-filter: blur(10px);">
                        
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 10px; border-bottom: 1px dashed rgba(255,255,255,0.05); padding-bottom:10px;">
                            <div style="display:flex; flex-direction: column; gap:2px;">
                                <span style="color:var(--text-muted); text-transform:uppercase; font-size:10px; letter-spacing:0.5px;">Fans</span>
                                <span style="color:var(--gold); font-weight:800; font-size:13px;">${(idol.fans || 0).toLocaleString()}</span>
                            </div>
                            <div style="display:flex; flex-direction: column; gap:2px; text-align:right;">
                                <span style="color:var(--text-muted); text-transform:uppercase; font-size:10px; letter-spacing:0.5px;">Fame</span>
                                <span style="color:var(--primary); font-weight:800; font-size:13px;">${idol.stats.fame || 0}</span>
                            </div>
                        </div>
                        ...
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 10px; border-bottom: 1px dashed rgba(255,255,255,0.05); padding-bottom:10px;">
                            <div style="display:flex; flex-direction: column; gap:2px;">
                                ...
                                <span style="color:#fff; font-weight:800; font-size:13px;">${idol.stats.visual || 0}</span>
                            </div>
                            <div style="display:flex; flex-direction: column; gap:2px; text-align:right;">
                                <span style="color:var(--text-muted); text-transform:uppercase; font-size:10px; letter-spacing:0.5px;">Risk</span>
                                <span style="color:var(--error); font-weight:800; font-size:13px;">${idol.scandalRisk || idol.stats.scandal_risk || 0}%</span>
                            </div>
                        </div>
                        ...
                        <div style="margin-top:auto; padding-top:4px;">
                            <div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:6px; font-weight:bold;">
                                <span style="color:var(--text-muted); text-transform:uppercase; font-size:10px; letter-spacing:1px;">Stress</span>
                                <span style="font-family: 'JetBrains Mono'; font-size:13px; color:${stress > 60 ? 'var(--error)' : (stress > 30 ? 'var(--gold)' : 'var(--success)')};">${stress}%</span>
                            </div>
                            <div style="background: rgba(0,0,0,0.6); height:4px; border-radius:2px; overflow:hidden; box-shadow: inset 0 1px 3px rgba(0,0,0,0.5);">
                                <div style="height:100%; width:${stress}%; background: ${stress > 60 ? 'var(--error)' : (stress > 30 ? 'var(--gold)' : 'var(--success)')}; box-shadow: 0 0 5px ${stress > 60 ? 'var(--error)' : 'transparent'}; transition: width 0.3s ease;"></div>
                            </div>
                        </div>
    ...
    openIdolProfile(id) {
        ...
        modalBody.innerHTML = `
            ...
            ${gameManager.state.techs && gameManager.state.techs.includes('tech_ai_manager') ? `
            <div style="margin-top: 24px; border-top: 1px solid var(--border-color); padding-top: 16px;">
                <h4 style="font-size: 14px; margin-bottom: 12px; color: var(--gold); display: flex; align-items: center; justify-content: space-between;">
                    🧠 AI TỰ TRỊ (AUTO-PLAY)
                    <div style="display:flex; align-items:center; gap:8px;">
                        <span style="font-size:10px; font-weight:bold; color:\${idol.autoAI ? 'var(--success)' : 'var(--text-muted)'};">\${idol.autoAI ? 'ON' : 'OFF'}</span>
                        <label style="position:relative; display:inline-block; width:40px; height:20px; cursor:pointer;">
                            <input type="checkbox" \${idol.autoAI ? 'checked' : ''} onchange="gameApp.toggleAutoAI('\${idol.id}', this.checked)" style="opacity:0; width:0; height:0; position:absolute;">
                            <span style="position:absolute; top:0; left:0; right:0; bottom:0; background-color:\${idol.autoAI ? 'var(--success)' : 'rgba(255,255,255,0.2)'}; transition:.4s; border-radius:20px;"></span>
                            <span style="position:absolute; height:16px; width:16px; left:2px; bottom:2px; background-color:white; transition:.4s; border-radius:50%; transform:\${idol.autoAI ? 'translateX(20px)' : 'translateX(0)'};"></span>
                        </label>
                    </div>
                </h4>
                <p style="font-size: 11px; color: var(--text-muted); font-style: italic;">Khi kích hoạt, Model tự đưa ra các quyết định trước Khủng hoảng/Sự kiện ngẫu nhiên theo tính cách.</p>
            </div>
            ` : ''}
            ...
            <div class="action-group mt-20" style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                ...
                <button class="btn-action w-full" style="background:#059669; color:white; border:none;" onclick="gameApp.closeIdolProfile(); gameApp.switchView('jobs'); gameApp.showToast('Vui lòng chọn hợp đồng cho ' + '${idol.name}', 'info');">🏢 GIAO VIỆC</button>
                ...
                <button class="btn-action w-full" style="background:#3b82f6; color:white; border:none;" onclick="gameApp.renameIdol('${idol.id}')">✏️ ĐỔI NGHỆ DANH</button>
    ...
    toggleAutoAI(idolId, value) {
        ...
        this.openIdolProfile(idolId);
        if(value) {
            this.showToast(`${idol.name} đã bật AI Tự Trị! Cô ấy sẽ tự phản ứng với sự kiện.`, "success");
        } else {
            this.showToast("Đã tắt AI Tự Trị. Bạn sẽ giải quyết khủng hoảng thủ công.", "info");
        }
    },
    ...
    renameIdol(id) {
        ...
        this.showDialog({
            title: "ĐỔI NGHỆ DANH",
            message: `Quyết định thay đổi nghệ danh cho ${idol.name}.<br><br><input type='text' id='new-idol-name' value='${idol.name}' class='w-full p-2' style='background:var(--bg-elevated); color:white; border:1px solid var(--border-color); border-radius:4px;'>`,
            type: "confirm",
            onConfirm: () => {
                const newNameInput = document.getElementById('new-idol-name');
                if (newNameInput && newNameInput.value.trim() !== "") {
                    const newName = newNameInput.value.trim();
                    if (newName !== idol.name) {
                        idol.name = newName;
                        dbManager.saveIdolData(idol);
                        this.showToast(`Đã thay đổi nghệ danh thành ${newName}!`, "success");
                        this.refreshUI(id);
                    }
                }
            }
    ...
    async openChat(id) {
        ...
        if (!idol.chatHistory) idol.chatHistory = [];
        
        if (idol.chatHistory.length > 0) {
            idol.chatHistory.forEach(msg => {
                this.appendMessage(msg.role, msg.text, null, false);
            });
            this.appendMessage('system', `--- Tiếp tục cuộc trò chuyện ---`, null, false);
        } else {
            // Greet dynamically based on mood and affinity only if first time
    ...
    async generateDynamicGreeting(idol) {
        ...
        try {
            ...
            const fetchPromise = apiManager.generateText(prompt);
    ...
    clearChat() {
        if (!this.currentChatIdol) return;
        this.currentChatIdol.chatHistory = [];
        dbManager.saveIdolData(this.currentChatIdol);
        const messagesBox = document.getElementById('chat-messages');
        messagesBox.innerHTML = '';
        this.appendMessage('system', `[Hệ thống] Đã xoá lịch sử trò chuyện.`, null, false);
        this.generateDynamicGreeting(this.currentChatIdol);
    },

    appendMessage(role, text, id = null, saveToHistory = true) {
        const messagesBox = document.getElementById('chat-messages');
        ...
        if (text === 'typing') {
            ...
            if (saveToHistory && this.currentChatIdol) {
                if (!this.currentChatIdol.chatHistory) this.currentChatIdol.chatHistory = [];
                this.currentChatIdol.chatHistory.push({ role, text });
                if (this.currentChatIdol.chatHistory.length > 20) {
                    this.currentChatIdol.chatHistory.shift(); // Keep only last 20 messages
                }
                dbManager.saveIdolData(this.currentChatIdol);
            }
        }
    ...
    async sendChatMessage() {
        ...
        const chatHistory = (idol.chatHistory || []).slice(-10).map(msg => {
            if (msg.role === 'system') return `Hệ thống: ${msg.text}`;
            return (msg.role === 'user' ? "Sếp: " : idol.name + ": ") + msg.text;
        }).join('\n');
...
PHẢN HỒI DUY NHẤT BẰNG RAW JSON:
{
   ...
   "image_prompt": "Nếu suggested_ui_actions có 'photo', DỰA BÁM SÁT VÀO NGỮ CẢNH CUỘC CHAT để suy luận ra 1 prompt tạo ảnh (Tiếng Anh, professional, FULL-BODY SHOT, 8k, cinematic, mô tả rõ trang phục người mẫu đang mặc, bối cảnh và cảm xúc lúc này). ƯU TIÊN GÓC CHỤP TOÀN THÂN (Full-body). Nếu không có photo action, để rỗng."
        ...
        try {
            ...
            fetchPromise = apiManager.generateText(promptText);
    ...
    // -- MỞ RỘNG KINH DOANH (5 ĐỀ XUẤT) --

    renderCinema() {
        const grid = document.getElementById('cinema-grid');
        if (!grid) return;
        const projects = [
            { id: 'film1', name: 'Love in Seoul (Rom-Com)', cost: 5000, reqVisual: 50, rewardFame: 200, emoji: '💕' },
            { id: 'film2', name: 'Cyberpunk Noir (Sci-Fi)', cost: 15000, reqVisual: 120, rewardFame: 500, emoji: '🦾' },
            { id: 'film3', name: 'The Last Empire (Epic History)', cost: 50000, reqVisual: 300, rewardFame: 2000, emoji: '👑' }
        ];
        
        let html = '';
        projects.forEach(p => {
            html += `
                <div class="shop-item" style="border-left: 4px solid #3b82f6;">
                    <div style="font-size:30px; margin-bottom:10px;">${p.emoji}</div>
                    <h4>${p.name}</h4>
                    <p style="font-size:12px; color:var(--text-muted);">Yêu cầu: Visual > ${p.reqVisual}<br>Kinh phí: <span style="color:var(--gold); font-weight:bold;">${p.cost.toLocaleString()} 💰</span></p>
                    <p style="color:var(--success); font-weight:bold; font-size:13px;">Phần thưởng: +${p.rewardFame} Fame</p>
                    <button class="btn-action w-full mt-10" style="background:#3b82f6; color:white;" onclick="gameApp.castCinema('${p.id}', ${p.cost}, ${p.reqVisual}, ${p.rewardFame}, '${p.name}')">CASTING GIAO VAI</button>
                </div>
            `;
        });
        grid.innerHTML = html;
    ...
    castCinema(id, cost, reqVisual, rewardFame, name) {
        if (gameManager.state.money < cost) return this.showToast("Không đủ kinh phí sản xuất!", "error");
        
        const idols = cardEngine.getAllIdols();
        if(idols.length === 0) return this.showToast("Chưa có Model!", "error");
        let opts = idols.map(i => `<option value="${i.id}">${i.name} (Visual: ${i.stats.visual})</option>`).join('');
        
        this.showDialog({
            title: `Casting Khởi Quay: ${name}`,
            message: `Chọn diễn viên chính mang trọng trách gánh doanh thu phim.<br><select id="cinema-select" class="w-full mt-10 p-2" style="background:var(--bg-elevated); color:white; border:1px solid var(--border-color);">${opts}</select>`,
            type: "confirm",
            onConfirm: () => {
                const idolId = document.getElementById('cinema-select').value;
                const idol = cardEngine.getIdol(idolId);
                if (idol.stats.visual < reqVisual) {
                    gameManager.state.money -= cost;
                    this.refreshUI();
                    return this.showDialog({title: "FLOP THẢM HẠI", message: `Đạo diễn thất vọng! ${idol.name} diễn xuất quá đơ, biểu cảm gượng gạo! Công ty lỗ trắng ${cost.toLocaleString()} 💰!`, type:"info"});
                }
                
                gameManager.state.money -= cost;
                idol.stats.fame += rewardFame;
                idol.fans = (idol.fans || 0) + (rewardFame * 50);
                dbManager.saveIdolData(idol);
                this.refreshUI();
                this.showDialog({title: "🎉 BLOCKBUSTER THÀNH CÔNG 🎉", message: `Bộ phim cháy vé trên toàn cầu! Diễn xuất của ${idol.name} được giới chuyên môn khen ngợi rần rần! +${rewardFame} Fame và Hàng ngàn Fan hâm mộ bùng nổ!`, type:"info"});
            }
        });
    },

    renderFashion() { }, // Layout handled in HTML structure

    startFashionWeek() {
        if (gameManager.state.money < 5000) return this.showToast("Bạn cần 5000 💰 để mua Slot diễn Runway!", "error");
        const idols = cardEngine.getAllIdols();
        if(idols.length === 0) return this.showToast("Bạn không có ai để diễn cat-walk cả!", "error");
        
        gameManager.state.money -= 5000;
        this.refreshUI();
        
        // Tỷ lệ may rủi dựa trên Visual
        const randomIdol = idols[Math.floor(Math.random() * idols.length)];
        const rivalVisual = Math.floor(Math.random() * 200) + 30; // Random rival difficulty
        
        let win = randomIdol.stats.visual >= rivalVisual;
        let p = win ? `<span style="color:var(--success); font-weight:bold; font-size:16px;">👑 CHAMPION!</span><br>Model <strong>${randomIdol.name}</strong> của bạn đè bẹp đối thủ với những bước Cat-walk thần sầu! (Visual ${randomIdol.stats.visual} đánh bại đối phương ${rivalVisual})<br><br><span style="color:var(--gold);">THƯỞNG: +50 💎, +15.000 💰</span>` : `<span style="color:var(--error); font-weight:bold; font-size:16px;">❌ THẤT BẠI.</span><br>Vedette công ty bạn bị đối thủ lấn lướt hoàn toàn trên sàn diễn! (Visual ${randomIdol.stats.visual} thua xa đối phương ${rivalVisual})<br>Hãy nâng cấp sắc đẹp Model và thử lại sau!`;
        
        if (win) {
            gameManager.state.diamonds += 50;
            gameManager.state.money += 15000;
            randomIdol.stats.fame += 50;
            dbManager.saveIdolData(randomIdol);
            this.refreshUI();
        }
        
        document.getElementById('fashion-active-show').innerHTML = `
            <div style="background:var(--bg-elevated); padding:24px; text-align:center; border: 2px dashed ${win ? 'var(--success)' : 'var(--error)'}; border-radius:8px; animation: modalPop 0.5s ease forwards;">
                <h3 style="margin-top:0; margin-bottom:20px; font-size:20px;">BẢNG ĐIỂM FASHION WEEK</h3>
                <img src="${randomIdol.avatarUrl && !randomIdol.avatarUrl.startsWith('blob:') ? randomIdol.avatarUrl : gameApp.fallbackImage}" style="width:100px; height:100px; border-radius:50%; object-fit:cover; border:3px solid ${win ? 'var(--success)' : 'var(--error)'}; margin-bottom:15px; box-shadow:0 0 15px rgba(255,255,255,0.2);">
                <p style="line-height:1.6; color:var(--text-main);">${p}</p>
            </div>
        `;
        document.getElementById('fashion-active-show').classList.remove('hidden');
    },
    ...
    renderPR() {
        ...
        if (!grid) return;
        const options = [
            { id: 'clean', name: 'Tẩy Trắng Báo Chí 🧼', desc: 'Sử dụng quan hệ xóa toàn bộ bài báo tiêu cực. Giảm tới 50% Rủi ro Scandal của Model.', cost: 3000 },
            { id: 'drama', name: 'Tạo Lùm Xùm Truyền Thông 🔥', desc: 'Bơm tin đồn hẹn hò giả mạo nhằm câu phốt. Tăng +60 Fame lập tức, nhưng tăng mạnh 20 Căng Thẳng (Stress).', cost: 1500 },
            { id: 'bot', name: 'Mua Fan Ảo & Seeding 🤖', desc: 'Chạy hệ thống Bot tăng Like. Bơm thẳng 15,000 Fan. Đổi lại Model dính +15 Rủi ro Scandal do bị bóc phốt mua Fan.', cost: 4000 }
        ];
        
        let html = '';
        options.forEach(o => {
            html += `
                <div class="shop-item" style="border-left: 4px solid var(--error);">
                    <h4 style="color:var(--error);">${o.name}</h4>
                    <p style="font-size:13px; color:var(--text-muted); line-height:1.5;">${o.desc}</p>
                    <button class="btn-action w-full mt-10" style="background:var(--error); color:white; border:none;" onclick="gameApp.buyPR('${o.id}', ${o.cost})">THỰC THI CHIẾN DỊCH (${o.cost.toLocaleString()} 💰)</button>
                </div>
            `;
        });
        grid.innerHTML = html;
    ...
    buyPR(type, cost) {
        if (gameManager.state.money < cost) return this.showToast("Công ty cạn ngân quỹ PR!", "error");
        const idols = cardEngine.getAllIdols();
        if(idols.length === 0) return this.showToast("Hãy chiêu mộ Model trước đã!", "error");
        
        let opts = idols.map(i => `<option value="${i.id}">${i.name} (Risk: ${i.scandalRisk||0}% - Fame: ${i.stats.fame})</option>`).join('');
        this.showDialog({
            title: `Chiến Dịch PR Media`,
            message: `Tiến hành áp dụng chiến lược này lên Gà Nhỏ nào?<br><select id="pr-select" class="w-full mt-10 p-2" style="background:var(--bg-elevated); color:white; border:1px solid var(--border-color);">${opts}</select>`,
            type: "confirm",
            onConfirm: () => {
                const idol = cardEngine.getIdol(document.getElementById('pr-select').value);
                gameManager.state.money -= cost;
                
                if (type === 'clean') {
                    idol.scandalRisk = Math.max(0, (idol.scandalRisk || 0) - 50);
                    idol.stats.scandal_risk = idol.scandalRisk;
                } else if (type === 'drama') {
                    idol.stats.fame += 60;
                    idol.stress = Math.min(100, (idol.stress || 0) + 20);
                } else if (type === 'bot') {
                    idol.fans = (idol.fans || 0) + 15000;
                    idol.scandalRisk = Math.min(100, (idol.scandalRisk || 0) + 15);
                    idol.stats.scandal_risk = idol.scandalRisk;
                }
                
                dbManager.saveIdolData(idol);
                this.refreshUI();
                this.showDialog({title:"THÀNH CÔNG", message: "Phòng Truyền thông & Báo chí đã xử lý xong yêu cầu!", type:"info"});
            }
        });
    },

    renderSponsors() {
        if (!gameManager.state.sponsors) gameManager.state.sponsors = [];
        const grid = document.getElementById('sponsor-grid');
        if (!grid) return;
        
        // Count total fame across all models
        let totalFame = cardEngine.getAllIdols().reduce((acc, i) => acc + (i.stats.fame || 0), 0);
        
        const sp = [
            { id: 'sp_brand', name: 'Đại Diện Brand Giải Khát🥤', reqFame: 500, reward: "Nhận Quỹ Tài Trợ Tức Thời: 8,000 💰", claimed: gameManager.state.sponsors.includes('sp_brand') },
            { id: 'sp_makeup', name: 'Đại Sứ Thương Hiệu Mỹ Phẩm💄', reqFame: 1500, reward: "Quỹ Phát Triển Sắc Đẹp: 200 💎, 10,000 💰", claimed: gameManager.state.sponsors.includes('sp_makeup') },
            { id: 'sp_global', name: 'Chuyên Cơ Thời Trang Global✈️', reqFame: 4000, reward: "Quyền Lực Tối Thượng: 500 💎, 50,000 💰", claimed: gameManager.state.sponsors.includes('sp_global') },
        ];
        
        let html = `<div style="grid-column: 1/-1; background:rgba(212, 175, 55, 0.1); padding:16px; border-radius:8px; border:1px dashed var(--gold); margin-bottom:15px;">
                        <h4 style="margin:0; color:var(--text-main);">TỔNG DANH TIẾNG TẬP ĐOÀN (FAME): <span style="color:var(--gold); font-size:24px; vertical-align:middle;">${totalFame}</span> 🔥</h4>
                        <p style="margin:5px 0 0; font-size:13px; color:var(--text-muted);">Sưu tập Fame từ mọi Model để thu hút những Hợp đồng Đầu tư béo bở nhất giới Showbiz!</p>
                    </div>`;
        
        sp.forEach(s => {
            let isClaimable = totalFame >= s.reqFame;
            html += `
                <div class="shop-item" style="border: 1px solid ${s.claimed ? 'var(--success)' : (isClaimable ? 'var(--gold)' : 'var(--border-color)')}; background: ${s.claimed ? 'rgba(16, 185, 129, 0.1)' : 'var(--bg-surface)'};">
                    <h4 style="color:${s.claimed ? 'var(--success)' : 'var(--text-main)'};">${s.name}</h4>
                    <p style="font-size:13px; color:var(--success); font-weight:bold; margin-top:10px;">${s.reward}</p>
                    <p style="font-size:12px; color:var(--text-muted); margin-top:5px;">Yêu cầu: Đạt mốc ${s.reqFame.toLocaleString()} Fame</p>
                    
                    ${s.claimed ? 
                        `<button class="btn-action w-full mt-10" disabled style="background:transparent; color:var(--success); border:1px solid var(--success);">ĐÃ KÝ ĐỘC QUYỀN ✅</button>` : 
                        `<button class="btn-action btn-primary w-full mt-10" ${!isClaimable ? 'disabled style="filter:grayscale(1); opacity:0.5;"' : ''} onclick="gameApp.claimSponsor('${s.id}', ${totalFame}, ${s.reqFame})">TIẾN HÀNH KÝ KẾT</button>`
                    }
                </div>
            `;
        });
    ...
    claimSponsor(id, current, req) {
        if (current < req) return this.showToast("Danh tiếng Tập đoàn chưa đủ tầm!", "error");
        
        gameManager.state.sponsors.push(id);
        
        if (id === 'sp_brand') { gameManager.state.money += 8000; }
        if (id === 'sp_makeup') { gameManager.state.money += 10000; gameManager.state.diamonds += 200; }
        if (id === 'sp_global') { gameManager.state.money += 50000; gameManager.state.diamonds += 500; }
        
        this.saveSettings();
        this.renderSponsors();
        this.refreshUI();
        
        this.showDialog({title:"KÝ KẾT THÀNH CÔNG", message: "Các Nhà Tài Trợ vô cùng hài lòng với danh tiếng của Công ty bạn! Tiền mặt & Kim Cương đã được rót vào tài khoản.", type:"info"});
    },

    renderTechTree() {
        if (!gameManager.state.techs) gameManager.state.techs = [];
        const grid = document.getElementById('techtree-grid');
        if (!grid) return;
        
        const techs = [
            { id: 'tech_network', name: 'Mạng Lưới Underworld ♠️', cost: 100, desc: 'Dùng Kim Cương để hối lộ các quan chức. (KÍCH HOẠT NHẬN TỨC THỜI: Toàn bộ Model giảm 20 Độ Tha Hóa (Corruption) & Giảm 20 Rủi ro Scandal). Mua được 1 lần.', claimed: gameManager.state.techs.includes('tech_network') },
            { id: 'tech_trainer', name: 'Đội Ngũ Trainer Thượng Đỉnh 🏋️', cost: 150, desc: 'Thuê các bậc thầy đào tạo từ Kinh đô Thời trang. (KÍCH HOẠT NHẬN TỨC THỜI: Toàn bộ Model tăng +25 Visual).', claimed: gameManager.state.techs.includes('tech_trainer') },
            { id: 'tech_studio', name: 'Nâng Cấp Thiết Bị Studio 📸', cost: 200, desc: 'Cài đặt hệ thống ánh sáng Pro. (KÍCH HOẠT NHẬN TỨC THỜI: Toàn bộ Model Tăng +1 Level và Phục hồi Stress về 0).', claimed: gameManager.state.techs.includes('tech_studio') },
            { 
                id: 'tech_ai_manager', 
                name: 'Siêu Trí Tuệ Nhân Tạo (Q-Star) 🧠', 
                cost: 500, 
                desc: 'Mở khóa tính năng Mô Hình Tự Trị. Cho phép Model tự đưa ra quyết định, phản ứng qua AI Model hoặc thủ công. YÊU CẦU: Một Model Level >= 10, Fame >= 5000 và 100,000 Tài chính.', 
                claimed: gameManager.state.techs.includes('tech_ai_manager'),
                req: () => {
                    let moneyOk = gameManager.state.money >= 100000;
                    let modelOk = cardEngine.getAllIdols().some(i => (i.level || 1) >= 10 && (i.stats.fame || 0) >= 5000);
                    return moneyOk && modelOk;
                }
            }
        ];
        
        let html = `<div style="grid-column:1/-1; margin-bottom:15px; color:var(--text-muted); font-style:italic;">Hệ thống Tech Tree (Nghiên cứu) hiện tại hoạt động theo Cơ chế Nhận Lợi Ích Tức Thì khi nâng cấp, áp dụng cho toàn bộ Model đang có trong công ty!</div>`;
        techs.forEach(t => {
            let disabled = "";
            let btnText = `NGHIÊN CỨU (${t.cost} 💎)`;
            if (t.req && !t.req()) {
                disabled = "disabled";
                btnText = "CHƯA ĐẠT ĐIỀU KIỆN";
            }
            html += `
                <div class="shop-item ${t.claimed ? 'bg-purple-900 border-purple-500' : ''}" style="border-left: 4px solid var(--accent);">
                    <h4 style="color:var(--gold);">${t.name}</h4>
                    <p style="font-size:13px; color:var(--text-main); margin:10px 0;">${t.desc}</p>
                    ${t.claimed ? 
                        `<button class="btn-action w-full mt-10" disabled style="background:var(--bg-elevated); color:var(--success); border:1px solid var(--success);">ĐÃ KÍCH HOẠT</button>` : 
                        `<button class="btn-action w-full mt-10" ${disabled} style="${disabled ? 'background:var(--bg-surface); opacity:0.5; color:gray;' : 'background:var(--bg-surface); color:var(--gold); border:1px dashed var(--gold);'}" onclick="gameApp.buyTech('${t.id}', ${t.cost})">${btnText}</button>`
                    }
                </div>
            `;
        });
    ...
    buyTech(id, cost) {
        if (gameManager.state.diamonds < cost) return this.showToast("Không đủ lượng 💎 Kim cương yêu cầu!", "error");
        
        this.showDialog({
            title: `XÁC NHẬN NGHIÊN CỨU`,
            message: `Bạn sẵn sàng bạo chi ${cost} 💎 để kích hoạt Gói Công Nghệ này chứ? Hiệu ứng sẽ áp dụng lên toàn bộ Model đang có ngay lập tức!`,
            type: "confirm",
            onConfirm: () => {
                gameManager.state.diamonds -= cost;
                gameManager.state.techs.push(id);
                ...
                if (id === 'tech_network') {
                    idols.forEach(idol => {
                        idol.corruption = Math.max(0, (idol.corruption || 0) - 20);
                        idol.scandalRisk = Math.max(0, (idol.scandalRisk || 0) - 20);
                        idol.stats.scandal_risk = idol.scandalRisk;
                        dbManager.saveIdolData(idol);
                    });
                } else if (id === 'tech_trainer') {
                    idols.forEach(idol => {
                        idol.stats.visual = Math.min(100, (idol.stats.visual || 0) + 25);
                        dbManager.saveIdolData(idol);
                    });
                } else if (id === 'tech_studio') {
                    idols.forEach(idol => {
                        idol.level = (idol.level || 1) + 1;
                        idol.stress = 0;
                        dbManager.saveIdolData(idol);
                ...
                this.showDialog({title:"NÂNG CẤP HOÀN TẤT", message: "Toàn bộ Công Ty đã được thăng hạng! Kiểm tra tình trạng các Model để thấy biểu đồ chỉ số được cải thiện.", type:"info"});
            }
        });
    },

    renderSubsidiary() {
        if (!gameManager.state.subsidiary) {
            gameManager.state.subsidiary = { unlocked: false, roles: { director: null, pr: null, sales: null }, pendingProfit: 0 };
        }
        
        const grid = document.getElementById('subsidiary-grid');
        if (!grid) return;
        
        let sub = gameManager.state.subsidiary;
        
        if (!sub.unlocked) {
            grid.innerHTML = `
                <div style="grid-column:1/-1; text-align:center; padding: 40px; background:var(--bg-elevated); border-radius:12px; border:2px dashed var(--border-color);">
                    <div style="font-size:50px; margin-bottom:15px;">🏢</div>
                    <h3 style="color:var(--text-main); margin-bottom:10px;">THÀNH LẬP CÔNG TY CON</h3>
                    <p style="color:var(--text-muted); font-size:14px; margin-bottom:25px; line-height:1.6;">
                        Tạo ra một cỗ máy sinh lời thụ động.
                        Bạn cần tối thiểu <strong>3 Model</strong> trong đội hình và khoản phí <strong>100,000 💰 + 100 💎</strong> để thành lập.
                    </p>
                    <button class="btn-action w-full" style="background:var(--primary); color:black; font-weight:bold; font-size:16px; padding:15px; border-radius:8px;" onclick="gameApp.unlockSubsidiary()">THÀNH LẬP NGAY (100k 💰, 100 💎)</button>
                </div>
            `;
            return;
        }
        ...
        const roles = [
            { key: 'director', name: 'Giám Đốc Chi Nhánh', icon: '👔', desc: 'Quyết định hiệu suất hệ số lợi nhuận chung. Rate = Visual + Level', color: '#b45309' },
            { key: 'pr', name: 'Trưởng Phòng PR', icon: '🎤', desc: 'Có tỷ lệ tự động giảm Căng thẳng cho toàn kho Model. Phụ thuộc Level', color: '#ec4899' },
            { key: 'sales', name: 'Trưởng Phòng Sale', icon: '📊', desc: 'Quyết định số tiền cơ sở sinh ra mỗi chu kỳ. Phụ thuộc Fame', color: '#10b981' }
        ];
        ...
        roles.forEach(r => {
            const currentIdolId = sub.roles[r.key];
            const idol = currentIdolId ? cardEngine.getIdol(currentIdolId) : null;
            
            html += `
                <div class="shop-item" style="border-top: 4px solid ${r.color}; position:relative;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                        <span style="font-size:30px;">${r.icon}</span>
                        <span style="font-size:12px; font-weight:bold; color:${r.color}; padding:4px 8px; background:rgba(255,255,255,0.05); border-radius:12px;">${r.name}</span>
                    </div>
                    ${idol ? `
                        <div style="text-align:center; padding:10px; background:rgba(0,0,0,0.2); border-radius:8px; border:1px dashed ${r.color};">
                            <img src="${idol.avatarUrl && !idol.avatarUrl.startsWith('blob:') ? idol.avatarUrl : gameApp.fallbackImage}" style="width:60px; height:60px; border-radius:50%; object-fit:cover; border:2px solid ${r.color}; margin-bottom:5px;">
                            <h4 style="color:var(--gold); margin:5px 0;">${idol.name}</h4>
                            <div style="font-size:11px; color:var(--text-muted);">Lv.${idol.level || 1} | Visual:${idol.stats.visual} | Fame:${idol.stats.fame}</div>
                            <button class="btn-action w-full mt-10" style="background:var(--error); color:white; font-size:12px;" onclick="gameApp.unassignSubsidiary('${r.key}')">RÚT KHỎI VỊ TRÍ</button>
                        </div>
                    ` : `
                        <div style="text-align:center; padding:15px 10px; border:1px dashed var(--text-muted); border-radius:8px; opacity:0.6;">
                            <div style="margin-bottom:10px;"><em>Chưa bổ nhiệm</em></div>
                            <button class="btn-action w-full" style="background:var(--bg-elevated); color:white; border:1px solid ${r.color};" onclick="gameApp.openAssignSubsidiary('${r.key}')">+ BỔ NHIỆM</button>
                        </div>
                    `}
                    <div style="font-size:12px; color:var(--text-muted); margin-top:15px; line-height:1.4;">${r.desc}</div>
                </div>
            `;
        });
        ...
        // The bank area
        html += `
            <div style="grid-column:1/-1; background:linear-gradient(135deg, rgba(20,20,30,0.8), rgba(0,0,0,0.9)); padding:20px; border-radius:12px; border:1px solid var(--gold); display:flex; justify-content:space-between; align-items:center; box-shadow: 0 4px 15px rgba(212,175,55,0.15);">
                <div>
                    <h3 style="color:var(--primary); margin:0 0 5px 0; font-size:16px;">KÉT SẮT CÔNG TY CON</h3>
                    <p style="color:var(--text-muted); font-size:13px; margin:0;">Doanh thu thụ động sinh ra từ các chức vụ đã được bố trí mỗi khi bạn Tương tác (Hub/Studio/etc).</p>
                </div>
                <div style="text-align:right;">
                    <div style="color:var(--success); font-size:26px; font-weight:bold; font-family:'JetBrains Mono'; margin-bottom:10px;">${(sub.pendingProfit||0).toLocaleString()} 💰</div>
                    <button class="btn-action" style="background:var(--success); color:black; font-weight:bold;" onclick="gameApp.claimSubsidiaryProfit()">RÚT TIỀN</button>
                </div>
            </div>
        `;
    ...
    unlockSubsidiary() {
        if (cardEngine.getAllIdols().length < 3) return this.showToast("Cần tối thiểu 3 Model mới có thể đảm nhận các vị trí!", "error");
        if (gameManager.state.money < 100000 || gameManager.state.diamonds < 100) return this.showToast("Không đủ ngân sách (100k 💰, 100 💎)!", "error");
        
        gameManager.state.money -= 100000;
        gameManager.state.diamonds -= 100;
        gameManager.state.subsidiary.unlocked = true;
        this.saveSettings();
        this.refreshUI();
        this.renderSubsidiary();
        this.showDialog({title:"THÀNH LẬP THÀNH CÔNG", message: "Công ty con đã được thành lập! Hãy nhanh chóng bổ nhiệm các Trưởng phòng để bắt đầu càn quét túi tiền thiên hạ.", type:"info"});
    },

    openAssignSubsidiary(roleKey) {
        const idols = cardEngine.getAllIdols();
        const sub = gameManager.state.subsidiary;
        
        // Filter out those already assigned
        const assignedIds = Object.values(sub.roles).filter(id => id !== null);
        const availableIdols = idols.filter(i => !assignedIds.includes(i.id));
        
        if(availableIdols.length === 0) return this.showToast("Không có Model rảnh rỗi!", "error");
        
        let opts = availableIdols.map(i => `<option value="${i.id}">${i.name} (Lv.${i.level||1} - Vis:${i.stats.visual} - Fame:${i.stats.fame})</option>`).join('');
        this.showDialog({
            title: `Bổ Nhiệm Vị Trí`,
            message: `Quyết định đưa Model nào lên ghế nóng?<br><select id="sub-assign-sel" class="w-full mt-10 p-2" style="background:var(--bg-elevated); color:white; border:1px solid var(--border-color);">${opts}</select>`,
            type: "confirm",
            onConfirm: () => {
                const idolId = document.getElementById('sub-assign-sel').value;
                gameManager.state.subsidiary.roles[roleKey] = idolId;
                this.saveSettings();
                this.renderSubsidiary();
            }
        });
    ...
    unassignSubsidiary(roleKey) {
        gameManager.state.subsidiary.roles[roleKey] = null;
        this.saveSettings();
        this.renderSubsidiary();
    },

    processSubsidiaryCycle() {
        if (!gameManager.state.subsidiary || !gameManager.state.subsidiary.unlocked) return;
        const sub = gameManager.state.subsidiary;
        let earned = 0;
        let directorBonus = 1.0;
        
        // Calculate basic revenue structure and bonuses
        const dir = sub.roles.director ? cardEngine.getIdol(sub.roles.director) : null;
        const sales = sub.roles.sales ? cardEngine.getIdol(sub.roles.sales) : null;
        const pr = sub.roles.pr ? cardEngine.getIdol(sub.roles.pr) : null;
        
        if (dir) {
            directorBonus += ((dir.stats.visual + (dir.level || 1)*20) / 2000); // Small multiplier
            directorBonus = Math.min(2.0, directorBonus); // max double
        }
        
        if (sales) {
            earned = 50 + Math.floor(sales.stats.fame * 0.1); 
        }
        
        if (pr) {
            // Chance to reduce corruption/scandal/stress globally
            if (Math.random() < 0.1) {
                const idols = cardEngine.getAllIdols();
                let chosen = idols[Math.floor(Math.random()*idols.length)];
                chosen.stress = Math.max(0, (chosen.stress||0) - 2);
                dbManager.saveIdolData(chosen);
            }
        }
        
        if (earned > 0) {
            sub.pendingProfit = (sub.pendingProfit || 0) + Math.floor(earned * directorBonus);
        }
        
        // Save handled by incrementAction wrapper
    },

    claimSubsidiaryProfit() {
        const sub = gameManager.state.subsidiary;
        if (!sub || !sub.unlocked || !sub.pendingProfit) return this.showToast("Két sắt trống rỗng!", "error");
        
        const claim = sub.pendingProfit;
        gameManager.state.money += claim;
        sub.pendingProfit = 0;
        this.saveSettings();
        this.refreshUI();
        this.renderSubsidiary();
        this.showToast(`Đã thu hồi ${claim.toLocaleString()} 💰 doanh thu từ Công Ty Con!`, "success");
    },
    ...
    renderStudioSelect() {
        ...
        const accContainer = document.getElementById('studio-accessory-list');
        if (accContainer && gameManager.state.inventory) {
            let html = '';
            Object.keys(gameManager.state.inventory).forEach(itemId => {
                const qty = gameManager.state.inventory[itemId];
                if (qty > 0) {
                    const item = gameManager.shopItems.find(i => i.id === itemId);
                    // Filter accessories based on category/type
                    if (item && (item.category === 'beauty' || item.category === 'gift' || item.category === 'premium')) {
                        html += `
                        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; color: var(--gold); font-size: 13px; background: rgba(0,0,0,0.3); padding: 5px 10px; border-radius: 4px; border: 1px solid rgba(212,175,55,0.2);">
                            <input type="checkbox" name="studio-inv-acc" value="${item.effect || item.name}" class="elegant-checkbox" style="accent-color: var(--gold);">
                            ${item.name} (${qty} Cái)
                        </label>`;
                    }
                }
            ...
            if (html) {
                ...
                accContainer.innerHTML = `<em style="font-size: 11px; color: var(--text-muted);">Bạn chưa có phụ kiện trang sức nào phù hợp trong túi đồ. Hãy mua sắm thêm tại Shop!</em>`;
            }
        }
    },
    ...
    async triggerUpdateIdol(id) {
        ...
        await cardEngine.updateIdolTrend(id);
        this.refreshUI();
    ...
    async inferNationality(id) {
        ...
        try {
            await cardEngine.inferIdolNationality(id);
            this.refreshUI();
            if (document.getElementById('idol-profile-modal').style.display === 'flex') {
                this.openIdolProfile(id);
            }
    ...
    applyFilter(filterString) {
        ...
        if (img) {
            ...
            this.currentRenderSession.filter = filterString !== 'none' ? filterString : null;
        }
    },

    async submitToMagazine() {
        if (!this.currentRenderSession) return;
        if (this.currentRenderSession.magazineSubmitted) {
            return this.showToast("Bức ảnh này đã được gửi cho tòa soạn rồi!", "warning");
        }
        
        const idol = cardEngine.getIdol(this.currentRenderSession.idolId);
        if (!idol) return;

        this.injectKeys();
        
        this.startTask('magazine', 'Đang gửi tòa soạn đánh giá...');
        const evaluatePrompt = `
        You are a harsh but fair Fashion Magazine Editor (Giới mộ điệu).
        Evaluate this photoshoot based on the art direction and technical specs:
        Prompt: ${this.currentRenderSession.prompt}
        Specs: ${this.currentRenderSession.specs}
        Model Fame: ${idol.stats.fame}
        
        Rules:
        Give a score from 1 to 100 based on creativity, lighting, and composition described. High scale fame means they are popular, you might give a small bump or be harsher.
        Respond ONLY in a compact JSON format without markdown wrapping, exactly like this:
        { "score": 85, "critique": "+ Lời nhận xét ngắn gọn tiếng việt" }
        `;

        try {
            const resultText = await apiManager.generateText(evaluatePrompt);
            let resultData;
            try {
                resultData = JSON.parse(resultText.replace(/```json/g, '').replace(/```/g, '').trim());
            } catch(e) {
                resultData = { score: Math.round(Math.random() * 40 + 40), critique: "Bức ảnh ở mức tạm ổn, chưa đủ sâu sắc." };
            }
            
            const score = resultData.score || 50;
            const isFeatured = score >= 85;
            
            this.currentRenderSession.magazineSubmitted = true;
            this.endTask('magazine');

            let fameGain = 0;
            let moneyGain = 0;

            if (isFeatured) {
                fameGain = Math.round(score * 1.5);
                moneyGain = score * 50;
                idol.stats.fame += fameGain;
                idol.fans = (idol.fans || 0) + (score * 1000);
                gameManager.updateMoney(moneyGain);
                dbManager.saveIdolData(idol);
                this.refreshUI();
                
                this.showDialog({
                    title: "🌟 BÌA TẠP CHÍ THÀNH CÔNG!",
                    message: `<div style="text-align: center;">
                        <h1 style="color:var(--gold); font-size:40px; margin: 0;">${score}/100</h1>
                        <p style="color:var(--success); font-weight:bold;">Tuyệt tác nghệ thuật! Bức ảnh đã được lên bìa tạp chí danh tiếng.</p>
                        <em style="color:var(--text-muted);">"${resultData.critique}"</em>
                        <br><br>
                        <span style="color:var(--primary);">+${fameGain} Fame</span> | <span style="color:var(--success);">+${moneyGain} 💰 Tiền bản quyền</span>
                    </div>`,
                    type: "info"
                });
            } else {
                this.showDialog({
                    title: "📉 TỪ CHỐI ĐĂNG BÁO",
                    message: `<div style="text-align: center;">
                        <h1 style="color:var(--error); font-size:40px; margin: 0;">${score}/100</h1>
                        <p style="color:var(--text-main);">Tòa soạn đánh giá bức ảnh này chưa đủ tầm để lên trang nhất.</p>
                        <em style="color:var(--text-muted);">"${resultData.critique}"</em>
                    </div>`,
                    type: "info"
                });
            ...
            this.endTask('magazine');
            this.showToast("Có lỗi khi liên hệ Tòa soạn. Hãy thử lại sau.", "error");
        }
    },
    ...
    async triggerPhotoshoot() {
        ...
        const ar = document.getElementById('studio-aspect-ratio');
        if (ar) {
            if (ar.value === '9:16') { width = 576; height = 1024; }
            else if (ar.value === '16:9') { width = 1024; height = 576; }
            else { width = 1024; height = 1024; }
        }

        const lens = document.getElementById('studio-lens')?.value || '';
        const lighting = document.getElementById('studio-lighting')?.value || '';
        const outfitPreset = document.getElementById('studio-outfit-preset')?.value || '';
        
        // Handle accessories
        const equippedItems = [];
        document.querySelectorAll('input[name="studio-inv-acc"]:checked').forEach(cb => {
            equippedItems.push(cb.value);
        });

        const cameraOverrides = { width, height, lens, lighting, outfitPreset, equippedItems };
        ...
        this.showDialog({
            ...
            onConfirm: async () => {
                ...
                const darkroomOverlay = document.getElementById('darkroom-overlay');
                ...
                if (darkroomOverlay) darkroomOverlay.style.display = 'flex';
                img.style.filter = "blur(20px)"; // Reset filter to blurred state for darkroom effect
                ...
                const result = await studioDirector.executePhotoshoot(id, concept, imageModel, cameraOverrides);

                if (darkroomOverlay) darkroomOverlay.style.display = 'none';
                ...
                if (result && result.imageUrl) {
                    ...
                    img.onload = () => {
                        ...
                        // Apply darkroom developing animation
                        setTimeout(() => {
                            img.style.filter = "blur(0px)";
                            img.style.opacity = "1";
                            // Play a shutter sound (simulated by timeout visual pop)
                        ...
                        this.currentRenderSession = { idolId: id, url: result.imageUrl, prompt: result.promptUsed, saved: false, specs: result.technicalSpecs };
...
Strictly output JSON only.`;
    ...
    async triggerRandomEvent() {
        ...
        let eventType = 'DAILY_LIFE';
        let rand = Math.random();
        
        if (highRiskIdols.length > 0 && rand > 0.6) {
        ...
        this.startTask('random_event', 'Hệ thống đang mô phỏng sự kiện...');
        
        let promptText = '';
        if (eventType === 'PAPARAZZI') {
            promptText = `Bạn là AI Truyền thông. Ngôi sao ${targetIdol.name} (Tâm trạng: ${targetIdol.mood || 'Bình thường'}, Danh tiếng: ${targetIdol.stats.fame}, Áp lực: ${targetIdol.stress||0}/100, Tha hóa: ${targetIdol.corruption||0}/100, Rủi ro: ${targetIdol.stats.scandal_risk}) vừa bị Paparazzi bắt gặp.
Đây là một scandal (tình ái, ảnh nóng, kiệt sức v.v. tùy thuộc vào chỉ số).
Tạo 1 sự kiện ngắn.
Output ONLY JSON:
{
  "title": "Tiêu đề Scandal",
  "content": "Paparazzi chụp được gì...",
  "options": [
    { "text": "Chi tiền Bịt miệng", "cost": 5000, "fameEffect": 0, "stressEffect": -10, "corruptionEffect": 15 },
    { "text": "Xin lỗi, trần tình", "cost": 500, "fameEffect": -50, "stressEffect": 30, "corruptionEffect": -10 }
  ]
}`;
        } else {
            promptText = `Bạn là Game Master. Ngôi sao ${targetIdol.name} (Tính cách: ${targetIdol.traits ? targetIdol.traits.join(',') : 'Bình thường'}, Tâm trạng: ${targetIdol.mood || 'Bình thường'}, Áp lực: ${targetIdol.stress||0}) vừa gặp một tình huống đời thường tại Showbiz (Cãi nhau với stylist, Có đạo diễn gạ gẫm, Fan cuồng tặng quà lạ...).
Tạo 1 sự kiện ngắn.
Output ONLY JSON:
{
  "title": "Sự Kiện Đời Thường",
  "content": "Sự việc xảy ra...",
  "options": [
    { "text": "Xử lý ôn hòa", "cost": 500, "fameEffect": 10, "stressEffect": -10, "corruptionEffect": -5 },
    { "text": "Xử lý cực đoan", "cost": 0, "fameEffect": -20, "stressEffect": 20, "corruptionEffect": 10 }
  ]
}`;
        }
        ...
        try {
            ...
            // ======================
            // AUTO AI FEATURE (Q-Star)
            // ======================
            if (targetIdol.autoAI && gameManager.state.techs && gameManager.state.techs.includes('tech_ai_manager')) {
                this.startTask('auto_resolve', 'AI Tự trị đang phân tích...');
                const autoPrompt = `Ngôi sao ${targetIdol.name} gặp tình huống: ${data.title}. ${data.content}
Tùy chọn:
${data.options.map((opt, i) => i + ". " + opt.text).join('\\n')}
Dựa vào tính cách của cô ấy, cô ấy sẽ TỰ ĐỘNG chọn option nào? Hãy cân nhắc ${targetIdol.name} có tính cách ${targetIdol.traits ? targetIdol.traits.join(',') : 'khó tính'} và trạng thái ${targetIdol.mood}.
Chỉ trả về 1 CHỮ SỐ duy nhất là index của option (ví dụ: 0 hoặc 1).`;
                let aiChoiceIdx = 0;
                try {
                    let aiRes = await apiManager.generateText(autoPrompt);
                    aiChoiceIdx = parseInt(aiRes.replace(/\\D/g, ''));
                    if(isNaN(aiChoiceIdx) || aiChoiceIdx < 0 || aiChoiceIdx >= data.options.length) aiChoiceIdx = 0;
                } catch(e) {}
                this.endTask('auto_resolve');

                const opt = data.options[aiChoiceIdx];
                if (gameManager.state.money < opt.cost) {
                    this.showToast(`[AUTO-AI] ${targetIdol.name} muốn chọn "${opt.text}" nhưng Công ty KHÔNG ĐỦ NGÂN SÁCH! (Cần ${opt.cost}$) Thất bại!`, "error");
                    targetIdol.stress = Math.min(100, (targetIdol.stress||0) + 10);
                } else {
                    gameManager.updateMoney(-opt.cost);
                    targetIdol.stats.fame = Math.max(0, targetIdol.stats.fame + opt.fameEffect);
                    targetIdol.fans = Math.max(0, (targetIdol.fans||0) + (opt.fameEffect * 10));
                    targetIdol.stress = Math.min(100, Math.max(0, (targetIdol.stress || 0) + opt.stressEffect));
                    targetIdol.corruption = Math.min(100, Math.max(0, (targetIdol.corruption || 0) + (opt.corruptionEffect || 0)));
                    this.showDialog({
                        title: `🤖 AI TỰ TRỊ: SỰ KIỆN XỬ LÝ!`,
                        message: `<strong>${targetIdol.name}</strong> đã tự động phản ứng với sự kiện: <strong>${data.title}</strong><br><br>
                        <em>${data.content}</em><br><br>
                        <strong style="color:var(--success);">✅ Lựa chọn của Model: "${opt.text}" (${opt.cost > 0 ? "Tốn " + opt.cost+"💰" : "Miễn phí"})</strong>`,
                        type: "info"
                    });
                }
                dbManager.saveIdolData(targetIdol);
                this.refreshUI();
                return;
            }
            ...
            this.showDialog({
                title: eventType === 'PAPARAZZI' ? `📸 <strong>DISPATCH LEAK!</strong>` : `🎭 <strong>SỰ KIỆN ĐỜI THƯỜNG</strong>`,
                ...
                <div style="font-weight:bold; color:var(--gold); margin-bottom:10px;">QUYẾT ĐỊNH XỬ LÝ:</div>
            ...
            console.error(e);
            this.endTask('random_event');
            this.showToast("Radar sự kiện bị nhiễu sóng...", "error");
        }
    },
