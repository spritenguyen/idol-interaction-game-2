        ...
        techs.forEach(t => {
            html += `
                <div class="shop-item ${t.claimed ? 'bg-purple-900 border-purple-500' : ''}" style="border-left: 4px solid var(--accent);">
                    ...
                    ${t.claimed ? 
                        `<button class="btn-action w-full mt-10" disabled style="background:var(--bg-elevated); color:var(--success); border:1px solid var(--success);">ĐÃ KÍCH HOẠT VÀ NHẬN BUFF</button>` : 
                        `<button class="btn-action w-full mt-10" style="background:var(--bg-surface); color:var(--gold); border:1px dashed var(--gold);" onclick="gameApp.buyTech('${t.id}', ${t.cost})">NGHIÊN CỨU (${t.cost} 💎)</button>`
                    }
                </div>
            `;
        });
