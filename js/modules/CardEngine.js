class CardEngine {
	...
	async generateAIModelProfile(query, provider, service, tier = 'street') {
        let existingNames = "";
...
Cấu trúc JSON chuẩn:
{
    ...
    "skills": {
        ...
        try {
            let fetchPromise = (provider === 'gemini') ? service.generateContent(systemPrompt, { json: true }) : service.generateText(systemPrompt);
            let response;
            try {
                response = await Promise.race([fetchPromise, timeoutPromise]);
            } catch (innerError) {
                if (provider === 'gemini' && innerError.message === 'GEMINI_QUOTA_EXCEEDED') {
                    console.warn("Gemini Quota Exceeded. Falling back to Pollinations API.");
                    if (typeof document !== 'undefined') {
                        document.dispatchEvent(new CustomEvent('gemini-quota-exhausted'));
                    }
                    response = await Promise.race([pollinationsService.generateText(systemPrompt), timeoutPromise]);
                } else {
                    throw innerError;
                }
            }
    ...
    async updateIdolTrend(id, provider, service) {
        ...
        try {
            let res;
            try {
                res = (provider === 'gemini') ? await service.generateContent(prompt) : await service.generateText(prompt);
            } catch (innerError) {
                if (provider === 'gemini' && innerError.message === 'GEMINI_QUOTA_EXCEEDED') {
                    if (typeof document !== 'undefined') document.dispatchEvent(new CustomEvent('gemini-quota-exhausted'));
                    res = await pollinationsService.generateText(prompt);
    ...
    async inferIdolNationality(id, provider, service) {
        const idol = this.roster.get(id);
...
Trả về MỘT TỪ DUY NHẤT chứa tên quốc gia hoặc vùng lãnh thổ bằng tiếng Anh (chỉ 1-2 từ, KHÔNG có dấu ngoặc kép hay giải thích). Ví dụ: Japanese`;
        ...
        try {
            let res;
            try {
                res = (provider === 'gemini') ? await service.generateContent(systemPrompt) : await service.generateText(systemPrompt);
            } catch (innerError) {
                if (provider === 'gemini' && innerError.message === 'GEMINI_QUOTA_EXCEEDED') {
                    if (typeof document !== 'undefined') document.dispatchEvent(new CustomEvent('gemini-quota-exhausted'));
                    res = await pollinationsService.generateText(systemPrompt);
                } else throw innerError;
            }
